...
class Qvntvs:
    ...
    def rectangular_potential_profile(self,electron=True, plot=True):
        ...
        if electron:
            ...
            V_general = np.zeros(self.n_intervals)
            V_general[self.well_start_index:self.well_end_index] = -self.V0_hole
    ...
    def triangular_potential_profile(self, electron = True, barrier_bending=True, plot=True, serial_print=False):
        ...
        if electron:
            ...
            if barrier_bending:
                V_general[self.well_start_index:self.well_end_index] = -self.e * self.force * (x_well - x_well[0])
                V_general[0:self.well_start_index] = -self.V0_hole - self.e * self.force * (self.x[0:self.well_start_index] - x_well[0])
                V_general[self.well_end_index:] = -self.V0_hole - self.e * self.force * (self.x[self.well_end_index:] - x_well[-1])
            else:
                V_general[self.well_start_index:self.well_end_index] = -self.e * self.force * (x_well - x_well[0])
    ...
    def hamiltonian_matrix(self, V_general, inv_mass = None, electron=True, plot=True):

        if inv_mass is None:
            if electron == True:
                inv_mass = self.inverse_mass_profile(electron=True, plot=False)
            elif electron == False:
                inv_mass = self.inverse_mass_profile(electron=False, plot=False)
    ...
    def eigen_equation(self, H, electron = True, plot=True, n_levels=4):
        ...
        if electron == False:
            energy_levels = self.V0_hole + energy_levels
        if plot == True:
